...
from gpt4_prompts import baseline_repair_solutions

from datasets import Dataset

def process_batch(batch_questions, batch_solutions, gpt4_repair, args, start_index, ds):
    repaired_batch = baseline_repair_solutions(batch_questions, batch_solutions, gpt4_repair)
    updated_items = []

    for j, repaired_solution in enumerate(repaired_batch):
        current_item = ds[start_index + j]
        input_output_pairs = eval(current_item['input_output'])
        
        try:
            repair_accuracy = check_correctness(repaired_solution, input_output_pairs, args.timeout, args.debug)
            repair_accuracy = sum(1 for r in repair_accuracy if r == True) / len(repair_accuracy)
        except:
            repair_accuracy = 0.0

        print(gpt4_repair.total_inference_cost)
        print(repair_accuracy)

        current_item['baseline_repaired_solution_accuracy'] = repair_accuracy
        current_item['baseline_repaired_solutions'] = repaired_solution
...
def corrupt_dataset(args, gpt4_repair):
    ...
    for i, item in enumerate(tqdm(ds)):
        ...
        batch_solutions.append(item['faulty_solutions'][0])

        if len(batch_questions) == args.budget or i == len(ds) - 1:
            batch_items = process_batch(batch_questions, batch_solutions, gpt4_repair, args, i - args.budget + 1, ds)
...
def main():
    ...
    parser.add_argument("--dataset_path_load", type=str, default="../../data/corrupted_apps_filtered_introductory_test.json", help="Path to the dataset file.")
    parser.add_argument("--dataset_path_save", type=str, default="../../data/repaired_apps_filtered_introductory_test.json", help="Path to the dataset file.")
    ...
    parser.add_argument("--temperature_corrupt", type=float, default=0)
    ...
    corrupt_dataset(args, gpt4_repair)
